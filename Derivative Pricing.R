...
Derivative_Security = function(S, K, N, r, d, u, type){
  ...
  if(type == "European_call"){
    ...
    for(i in (nrow(option_tree)):2){
      NO <- (1/(1+r))*(q*option_tree[i, seq(2, ncol(option_tree), by = 2)] + (1-q)*option_tree[i, seq(1, ncol(option_tree), by = 2)])
      
      print("NO")
      print(NO)
      
      stock_tree = stock_development(S, i-2, u, d)
      
      EX <- pmax(stock_tree - K,0)
      
      print("EX")
      print(EX)
      
      
      option_tree[i-1, 1:(2^(N-1))] <- pmax(EX, NO)
...
European_put4
European_put5 = Derivative_Security(4, 4, 4, (1/4), (1/2), 2, "American_call")
European_put5
